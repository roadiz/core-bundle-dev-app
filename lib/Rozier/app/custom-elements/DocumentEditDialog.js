...
export default class DocumentEditDialog extends RoadizElement {
                ...
                ${
                    rzButtonRenderer({
                        ...
                        attributes: {
                            ...
                            value: 'cancel',
                            closetarget: '',
                            'aria-label': window.RozierConfig?.messages?.close,
                        },
                    }).outerHTML
                }
            </header>
    ...
    setupEventListeners() {
        ...
        const editLink = this.dialog?.querySelector('.document-edit-dialog__edit-link')
        if (editLink) this.listen(editLink, 'click', this.onEditLinkClick)
    }
