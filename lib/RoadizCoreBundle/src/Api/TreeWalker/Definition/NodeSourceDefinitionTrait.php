...
trait NodeSourceDefinitionTrait
{
    ...
    protected function getQueryBuilder(
        ...
        if (1 === count($nodeTypes)) {
            ...
            $entityName = $this->nodeTypeClassLocator->getSourceEntityFullQualifiedClassName($nodeType);
        } else {
        ...
        if (NodesSources::class === $entityName) {
            $qb->andWhere($qb->expr()->orX(
                ...array_map(
                    fn (NodeType $nodeType) => $qb->expr()->isInstanceOf($alias, $this->nodeTypeClassLocator->getSourceEntityFullQualifiedClassName($nodeType)),
                    $nodeTypes
                )
            ));
        }
