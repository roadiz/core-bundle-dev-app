...
export default class StackNodeTree {
    ...
    treeAvailable() {
        return !!this.page.querySelector('.nodetree-widget')
    }
    ...
    async refreshNodeTree(rootNodeId, tagId, page) {
        ...
        const data = await this.fetchNodeTree(rootNodeId, page ? parseInt(page) : undefined, tagId ? parseInt(tagId) : undefined)

        if (data.nodeTree) {
            await window.Rozier.fadeOut(nodeTreeContainer)
            ...
            await window.Rozier.fadeIn(newNodeTree)
