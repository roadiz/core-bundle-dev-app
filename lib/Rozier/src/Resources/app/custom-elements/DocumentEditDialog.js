...
export default class DocumentEditDialog extends RoadizElement {
    ...
    setupEventListeners() {
        ...
        const buttons = this.dialog?.querySelectorAll(`${CSS_SELECTORS.DIALOG_CLOSE}, ${CSS_SELECTORS.DIALOG_ACTIONS}`)

        this.on(buttons, 'click', this.onButtonClick)
        this.on(this.dialog, 'close', this.onDialogClose)
    }
