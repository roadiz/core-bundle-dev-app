...
export default class CustomFormFieldsPosition {
    ...
    async onSortableChange(event, list, element) {
        ...
        const id = parseInt($element.data('field-id'))
        ...
        if ($sibling.length === 0) {
            ...
            postData.nextId = parseInt($sibling.data('field-id'))
        } else {
            postData.prevId = parseInt($sibling.data('field-id'))
        }

        const response = await fetch(
            window.RozierConfig.routes.customFormFieldPositionAjax,
            {
                method: 'POST',
                headers: {
