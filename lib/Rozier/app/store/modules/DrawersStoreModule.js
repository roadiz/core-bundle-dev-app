...
const actions = {
    ...
    drawersInitData(
        { commit },
        { drawer, entity, ids, filters, maxLength, minLength, entityName },
    ) {
        ...
        if (!ids || ids.length === 0 || !entity) {
            commit(DRAWERS_INIT_DATA_REQUEST_EMPTY, {
                drawer,
                maxLength,
                minLength,
            })
            ...
            DrawerApi.getItemsByIds(entity, fetchIds, filters)
                .then((result) => {
                    if (result && entityName === NODE_SOURCE_DOCUMENT_ENTITY) {
                        ...
                        commit(DRAWERS_INIT_DATA_REQUEST_SUCCESS, {
                            drawer,
                            result: resultDto,
                            maxLength,
                            minLength,
    ...
    drawersDropzoneButtonClick({ dispatch }, drawer) {
...
const mutations = {
    ...
    [DRAWERS_ADD_ITEM](state, { drawer, item }) {
    ...
    [DRAWERS_INIT_DATA_REQUEST_SUCCESS](
        state,
        { drawer, result, maxLength, minLength },
    ) {
