export default class Mobile {
    constructor() {
        ...
        this.adminMenuLinks = this.adminMenu.querySelectorAll(
            'a:not([target="_blank"])',
        )
        this.adminMenuNavParents = this.adminMenu.querySelectorAll('.uk-parent')
        this.adminMenuNavParentHandles = this.adminMenu.querySelectorAll(
            '.uk-parent > p[role="menuitem"]',
        )
        ...
        this.mainContentOverlay = document.getElementById(
        ...
        if (this.userPicture) {
            ...
            this.userPicture.classList.add('rz-no-ajax-link')
        }
        // Events
        this.menu.addEventListener('click', this.menuClick)
        this.adminMenuLinks.forEach((adminMenuLink) =>
            adminMenuLink.addEventListener('click', this.adminMenuLinkClick),
        )
        this.adminMenuNavParentHandles.forEach((adminMenuNavParentHandle) =>
            adminMenuNavParentHandle.addEventListener(
                'click',
                this.adminMenuNavParentClick,
            ),
        )
        ...
        this.treeWrapperLink?.addEventListener(
            'click',
            this.treeWrapperLinkClick,
        )
        this.userPicture?.addEventListener('click', this.userPictureClick)
        this.userActionsLink?.addEventListener(
            'click',
            this.userActionsLinkClick,
        )
        this.mainContentOverlay?.addEventListener(
            'click',
            this.mainContentOverlayClick,
        )
    ...
    adminMenuNavParentClick(e) {
         ...
         * @type {HTMLElement}
         */
        const adminMenuParent = e.currentTarget?.closest('.uk-parent')
        if (!adminMenuParent) {
            return false
        }
    ...
    treeWrapperLinkClick(e) {
        if (
            e.currentTarget.className.indexOf('tab-link') === -1 &&
            this.treeOpen
        ) {
            this.closeTree()
        }
    ...
    closeTree() {
        ...
        // Remove active class
        this.treeButton.classList.remove('active')
