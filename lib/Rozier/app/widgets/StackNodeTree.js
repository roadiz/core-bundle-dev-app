...
export default class StackNodeTree {
    constructor() {
        ...
        if (this.page) {
            this.quickAddNodeButtons = this.page.querySelectorAll(
                '.stack-tree-quick-creation button',
            )
     ...
     */
    getTranslationId() {
        this.nodeTree = this.page.querySelector('.rz-tree__list')
        const translationId = parseInt(
            this.nodeTree.getAttribute('data-translation-id'),
        )
    ...
    async refreshTree() {
        await this.page.querySelector('rz-tree').refreshNodeTree()
    }
    ...
    async onQuickAddClick(event) {
        ...
        const parentNodeId = parseInt(
            link.getAttribute('data-children-parent-node'),
        )
        let tagId = null
        ...
        try {
            ...
            console.error('Error while quick adding node', data)
            if (data.violations?.length) {
                    ...
                    window.dispatchEvent(
                        new CustomEvent('pushToast', {
                            detail: {
                                title: data.title,
                                message:
                                    violation.title || violation.propertyPath,
                                status: 'danger',
                            },
                        }),
                    )
                ...
                window.dispatchEvent(
                    new CustomEvent('pushToast', {
                        detail: {
                            message: data.error_message || data.details,
                            status: 'danger',
                        },
                    }),
                )
    ...
    async refreshNodeTree(rootNodeId, tagId, page) {
        ...
        try {
            ...
            if (data.nodeTree) {
                ...
                const newNodeTree = wrapper.querySelector('ul.root-tree')

                rootTree.replaceWith(newNodeTree)

                window.Rozier.initNestables()
                window.Rozier.bindMainTrees()
                window.Rozier.lazyload.bindAjaxLink()
                await fadeIn(nodeTreeContainer)
