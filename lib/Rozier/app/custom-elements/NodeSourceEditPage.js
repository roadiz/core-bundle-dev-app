...
export default class NodeSourceEditPage extends HTMLElement {
    constructor() {
        ...
        this.dropdown = null
        this.form = this.querySelector('#source')
        this.formRow = this.querySelectorAll('.rz-form-field')
            ...
            if (devName.getAttribute('data-dev-name') !== '') {
                ...
                if (label) {
                    label.insertAdjacentHTML(
                        'beforeend',
                        '<span class="field-dev-name">' +
                            devName.getAttribute('data-dev-name') +
                            '</span>',
                    )
                } else if (barLabel) {
    ...
    wrapInTabs() {
        ...
        let fields = this.querySelectorAll(
            '.rz-form-field[data-field-group-canonical]',
        )
        ...
        for (let i = 0; i < fieldsLength; i++) {
            ...
            let groupNameCanonical = fields[i].getAttribute(
        ...
        if (fieldsGroupsLength > 1) {
            this.form.insertAdjacentHTML(
                'beforeend',
                '<rz-tablist class="rz-tablist--underlined" id="node-source-form-switcher-nav-cont">' +
                    '<div class="rz-tablist__inner" role="tablist" id="node-source-form-switcher-nav">' +
                    '</div>' +
                    '</rz-tablist>' +
                    '<ul id="node-source-form-switcher">' +
                    '</ul>',
            )
            const formSwitcher = this.form.querySelector(
                '#node-source-form-switcher',
            )
            const formSwitcherNav = this.form.querySelector(
                '#node-source-form-switcher-nav',
            )

            for (let index in fieldGroups) {
                const fieldGroup = fieldGroups[index]
                const groupName2Safe = fieldGroup.id
                    .replace(/[\s_]/g, '-')
                    .replace(/[^\w-]+/g, '')
                const groupId = 'group-' + groupName2Safe
                const tabId = 'tab-' + groupName2Safe

                formSwitcher.insertAdjacentHTML(
                    'beforeend',
                    '<li role="tabpanel" aria-labelledby="' +
                        tabId +
                        '" class="rz-tabpanel field-group rz-form__field-list" id="' +
                        groupId +
                        '"></li>',
                )
                ...
                if (fieldGroup.id === 'default') {
                    formSwitcherNav.insertAdjacentHTML(
                        ...
                        '<button id="' +
                            tabId +
                            '" role="tab" class="rz-tablist__tab rz-tablist__tab--selected" type="button" aria-controls="' +
                            groupId +
                            '">Main</button>',
                    )
                } else {
                    formSwitcherNav.insertAdjacentHTML(
                        ...
                        '<button id="' +
                            tabId +
                            '" role="tab" class="rz-tablist__tab rz-tablist__tab" type="button" aria-controls="' +
                            groupId +
                            '">' +
                            fieldGroup.name +
                            '</button>',
        ...
        this.activateTabNav(this.form.querySelector('.rz-tablist__tab'))

        this.form.querySelectorAll('.rz-tablist__tab').forEach((button) => {
            button.addEventListener('click', () => {
                const tabId = button.getAttribute('aria-controls')

                this.form
                    .querySelectorAll('.rz-tablist__tab')
                    .forEach((item) => this.desactivateTabNav(item))
                this.activateTabNav(button.closest('.rz-tablist__tab'))

                this.form
                    .querySelectorAll('.field-group')
                    .forEach((group) => this.desactivateTab(group))
    ...
    async onFormSubmit(event) {
        ...
        try {
            ...
            if (data.public_url) {
                const publicUrlLinks =
                    document.querySelectorAll('a.public-url-link')
                publicUrlLinks.forEach((link) =>
                    link.setAttribute('href', data.public_url),
                )
            }
            if (data.preview_url) {
                const previewUrlLinks =
                    document.querySelectorAll('a.preview-url-link')
                previewUrlLinks.forEach((link) =>
                    link.setAttribute('href', data.preview_url),
                )
            }
        } catch (error) {
    ...
    displaySingleError(prefix, index, errors = []) {
        ...
        if (input) {
            const field = input.closest('.rz-form-field')
            if (field) {
    ...
    childrenNodeWidgetFlip(index) {
        if (index >= this.formRow.length - 2) {
            this.dropdown =
                this.formRow[index].querySelector('.uk-dropdown-small')
