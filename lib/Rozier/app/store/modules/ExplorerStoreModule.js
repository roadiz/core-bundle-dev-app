 ...
 */
const actions = {
    async explorerOpen({ commit, dispatch }, { entity, preFilters = null }) {
        ...
        // Open panel explorer
        commit(EXPLORER_OPEN, { entity, preFilters })
    ...
    explorerMakeSearch({ commit, state, getters }) {
        ...
        const preFilters = getters.getDrawerFilters || state.preFilters || null
...
const mutations = {
    ...
    [EXPLORER_OPEN](state, { entity, preFilters }) {
        state.isOpen = true
        state.isLoading = true
        state.entity = entity
        state.preFilters = preFilters
