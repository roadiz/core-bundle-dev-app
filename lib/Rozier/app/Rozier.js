...
export default class Rozier {
    constructor() {
        ...
        this.onNestableFolderTreeChange =
            this.onNestableFolderTreeChange.bind(this)
    ...
    onDocumentReady() {
        ...
        // --- Selectors --- //
        this.minifyTreePanelButton = document.querySelectorAll(
            '#minify-tree-panel-button',
        )
        this.mainTrees = document.querySelector('#main-trees')
        this.mainContentScrollable = document.querySelector(
    ...
    setupCollapsedNestableState() {
        if (window.localStorage) {
            this.collapsedNestableState = window.localStorage.getItem(
                'collapsed.uk.nestable',
            )
            /*
            ...
            this.collapsedNestableState = JSON.parse(
                this.collapsedNestableState,
            )
    ...
    syncCollapsedNestableState() {
        ...
        this.collapsedNestableState.folders.forEach((value) => {
            const li = document.querySelectorAll(
                '.uk-nestable-item[data-folder-id="' +
                    $.escapeSelector(value) +
                    '"]',
            )
            li.forEach((element) => {
    ...
    initNestables() {
        ...
        document.removeEventListener(
            'collapse.uk.nestable',
            this.onNestableCollapse,
        )
    ...
    bindMainTrees() {
         ...
         */
        const nodeTrees = document.querySelectorAll(
            '.nodetree-widget .root-tree',
        )
        ...
        const folderTrees = document.querySelectorAll(
            '.foldertree-widget .root-tree',
        )
    ...
    bindMainNodeTreeLangs() {
        document.body.addEventListener('click', (event) => {
            const target = event.target.closest(
                '#tree-container .nodetree-langs button',
            )
            if (target) {
    ...
    async refreshMainNodeTree(translationId = undefined) {
        const currentNodeTree = document.querySelector(
            '#tree-container .nodetree-widget',
        )
        if (!currentNodeTree) {
        ...
        try {
            ...
            if (typeof data.nodeTree !== 'undefined') {
                ...
                const newNodeTree = document.querySelector(
                    '#tree-container .nodetree-widget',
                )
                if (newNodeTree) {
    ...
    async refreshMainTagTree() {
        const currentTagTree = document.querySelector(
            '#tree-container .tagtree-widget',
        )
        if (!currentTagTree) {
        ...
        const url =
            window.RozierConfig.routes.tagsTreeAjax + '?' + query.toString()
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
        ...
        if (typeof data.tagTree !== 'undefined') {
            ...
            const newTagTree = document.querySelector(
                '#tree-container .tagtree-widget',
            )
            if (newTagTree) {
    ...
    async refreshMainFolderTree() {
        const currentFolderTree = document.querySelector(
            '#tree-container .foldertree-widget',
        )
        ...
        const url =
            window.RozierConfig.routes.foldersTreeAjax + '?' + query.toString()
        const response = await fetch(url, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
        ...
        if (typeof data.folderTree !== 'undefined') {
            ...
            const newFolderTree = document.querySelector(
                '#tree-container .foldertree-widget',
            )
            if (newFolderTree) {
    ...
    toggleTreesPanel() {
        document
            .getElementById('main-container-inner')
            .classList.toggle('trees-panel--minified')
        document.getElementById('main-content').classList.toggle('maximized')
        document
            .querySelector('#minify-tree-panel-button i')
            .classList.toggle('uk-icon-rz-panel-tree-open')
        document
            .getElementById('minify-tree-panel-area')
            .classList.toggle('tree-panel-hidden')
    ...
    openTreesPanel() {
        if (
            document
                .getElementById('main-container-inner')
                .classList.contains('trees-panel--minified')
        ) {
        ...
        if (detail[0]) {
            switch (true) {
                case detail[0].getAttribute('data-node-id') !== null:
                    this.collapsedNestableState.nodes.push(
                        detail[0].getAttribute('data-node-id'),
                    )
                    break
                case detail[0].getAttribute('data-tag-id') !== null:
                    this.collapsedNestableState.tags.push(
                        detail[0].getAttribute('data-tag-id'),
                    )
                    break
                case detail[0].getAttribute('data-folder-id') !== null:
                    this.collapsedNestableState.folders.push(
                        detail[0].getAttribute('data-folder-id'),
                    )
                    break
            }
            ...
            switch (true) {
                case detail[0].getAttribute('data-node-id') !== null:
                    this.collapsedNestableState.nodes.splice(
                        this.collapsedNestableState.nodes.indexOf(
                            detail[0].getAttribute('data-node-id'),
                        ),
                        1,
                    )
                ...
                case detail[0].getAttribute('data-tag-id') !== null:
                    this.collapsedNestableState.tags.splice(
                        this.collapsedNestableState.tags.indexOf(
                            detail[0].getAttribute('data-tag-id'),
                        ),
                        1,
                    )
                ...
                case detail[0].getAttribute('data-folder-id') !== null:
                    this.collapsedNestableState.folders.splice(
                        this.collapsedNestableState.folders.indexOf(
                            detail[0].getAttribute('data-folder-id'),
                        ),
                        1,
                    )
    ...
    async onNestableNodeTreeChange(event, rootEl, $element, status) {
        ...
        const childrenNodetree = element.parentElement.closest(
        ...
        if (nodeTree) {
            ...
            parentNodeId = parseInt(
                stackTree.getAttribute('data-parent-node-id'),
            )
        } else if (childrenNodetree) {
            parentNodeId = parseInt(
                childrenNodetree.getAttribute('data-parent-node-id'),
            )
        }
        ...
        if (
            nextElement &&
            typeof nextElement.getAttribute('data-node-id') !== 'undefined'
        ) {
            postData.nextNodeId = parseInt(
                nextElement.getAttribute('data-node-id'),
            )
        } else {
            const previousElement = element.previousElementSibling
            if (
                previousElement &&
                typeof previousElement.getAttribute('data-node-id') !==
                    'undefined'
            ) {
                postData.prevNodeId = parseInt(
                    previousElement.getAttribute('data-node-id'),
                )
            }
        }
        ...
        try {
            const response = await fetch(
                window.RozierConfig.routes.nodesPositionAjax,
                {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
    ...
    async onNestableTagTreeChange(event, rootEl, $element, status) {
        ...
        if (
            nextElement &&
            typeof nextElement.getAttribute('data-tag-id') !== 'undefined'
        ) {
            postData.nextTagId = parseInt(
                nextElement.getAttribute('data-tag-id'),
            )
        } else {
            const previousElement = element.previousElementSibling
            if (
                previousElement &&
                typeof previousElement.getAttribute('data-tag-id') !==
                    'undefined'
            ) {
                postData.prevTagId = parseInt(
                    previousElement.getAttribute('data-tag-id'),
                )
            }
        }
        ...
        try {
            const response = await fetch(
                window.RozierConfig.routes.tagPositionAjax,
                {
                    method: 'POST',
                    headers: {
                        Accept: 'application/json',
    ...
    async onNestableFolderTreeChange(event, rootEl, $element, status) {
        ...
        const folderTreeElement = element.parentElement.closest(
            '.foldertree-element',
        )
        const rootTreeElement = element.parentElement.closest('.root-tree')
        if (folderTreeElement) {
            parentFolderId = parseInt(
                folderTreeElement.getAttribute('data-folder-id'),
            )
        } else if (rootTreeElement) {
            parentFolderId = parseInt(
                rootTreeElement.getAttribute('data-parent-folder-id'),
            )
        }
        ...
        if (
            nextElement &&
            typeof nextElement.getAttribute('data-folder-id') !== 'undefined'
        ) {
            postData.nextFolderId = parseInt(
                nextElement.getAttribute('data-folder-id'),
            )
        } else {
            const previousElement = element.previousElementSibling
            if (
                previousElement &&
                typeof previousElement.getAttribute('data-folder-id') !==
                    'undefined'
            ) {
                postData.prevFolderId = parseInt(
                    previousElement.getAttribute('data-folder-id'),
                )
            }
        }
        ...
        try {
            const response = await fetch(
                window.RozierConfig.routes.folderAjaxEdit.replace(
                    '%folderId%',
                    folderId,
                ),
                {
    ...
    backTopBtnClick() {
        this.mainContentScrollable?.scrollIntoView({
            behavior: 'smooth',
            block: 'start',
        })
    }
