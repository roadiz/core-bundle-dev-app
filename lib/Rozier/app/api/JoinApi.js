...
export function getJoinsByIds({ ids = [], filters }) {
    ...
    return fetch(
        window.RozierConfig.routes.joinsAjaxByArray +
            '?' +
 ...
 */
export function getJoins({ searchTerms, preFilters, filters, moreData }) {
    const postData = {
        _token: window.RozierConfig.ajaxToken,
    ...
    return fetch(
        window.RozierConfig.routes.joinsAjaxExplorer +
            '?' +
            new URLSearchParams(postData),
        {
            method: 'GET',
