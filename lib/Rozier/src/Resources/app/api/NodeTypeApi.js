...
export function getNodeTypesByIds({ ids = [] }) {
    ...
    return fetch(window.RozierRoot.routes.nodeTypesAjaxByArray + '?' + new URLSearchParams(postData), {
        method: 'GET',
        headers: {
...
export function getNodeTypes({ searchTerms, preFilters, filters, filterExplorerSelection, moreData }) {
    ...
    return fetch(window.RozierRoot.routes.nodeTypesAjaxExplorer + '?' + new URLSearchParams(postData), {
        method: 'GET',
        headers: {
            Accept: 'application/json',
        },
