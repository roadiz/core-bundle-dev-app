...
final readonly class NodesSourcesWorkflowGuardListener implements EventSubscriberInterface
{
    ...
    {
        return [
            'workflow.nodesSources.guard' => ['guard'],
            'workflow.nodesSources.guard.publish' => ['guardPublish'],
            'workflow.nodesSources.guard.delete' => ['guardDelete'],
    ...
    {
        /** @var Node|NodesSources $object */
        $object = $event->getSubject();

        if ($object instanceof NodesSources && $object->getNode()->isLocked()) {
            $event->addTransitionBlocker(new TransitionBlocker(
                'A locked node cannot be deleted.',
                '1'
            ));
        } elseif ($object instanceof Node && $object->isLocked()) {
            $event->addTransitionBlocker(new TransitionBlocker(
                'A locked node cannot be deleted.',
                '1'
            ));
