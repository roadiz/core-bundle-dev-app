 ...
 */
export function defineLazyElement(name: string, loader: () => Promise<ElementModule>): void {
  ...
  async function load(tagName: string) {
    if (!loadingPromise) {
      loadingPromise = loader().then((module) => {
        ...
        const options = tagName === name ? undefined : { extends: tagName }
      ...
      for (const mutation of mutations) {
        for (const node of mutation.addedNodes) {
          if (node instanceof Element) {
            ...
            if (tagName === name || node.getAttribute('is') === name) {
              load(tagName)
            }
            else {
              const childElement = node.querySelector(selector)

              if (childElement) load(childElement.tagName.toLowerCase())
            }
          }
        }
      }
