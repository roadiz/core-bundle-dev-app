...
export default class DocumentAlignmentWidget extends RoadizElement {
    ...
    initializeOriginalHotspot() {
        ...
        if (originalHotspot) {
            try {
                ...
                console.error(
                    'DocumentAlignmentWidget: Invalid original hotspot data:',
                    error,
                )
    ...
    initializeOverridableHotspot() {
        this.hotspotOverridable = Boolean(
            this.getAttribute('hotspot-overridable'),
        )
    }
    ...
    setupOverrideHandling() {
        ...
        this.overrideButton.classList.add(
            DocumentAlignmentWidget.CSS_CLASSES.OVERRIDE_VISIBLE,
        )
        this.overrideButtonInput.checked = !!storedData
        this.area.inert = !this.overrideButtonInput.checked

        this.listen(this.overrideButtonInput, 'change', this.onOverrideChange)
    }
    ...
    initializeElements() {
        ...
        this.listen(this.resetButton, 'click', this.onResetClick)
    ...
    getImageElement() {
        if (!this.imageElement) {
            this.imageElement = this.querySelector(
                DocumentAlignmentWidget.CSS_SELECTORS.IMAGE,
            )
        }
    ...
    toggleResetButtonVisibility(isVisible) {
        this.resetButton?.classList.toggle(
            DocumentAlignmentWidget.CSS_CLASSES.RESET_VISIBLE,
            isVisible,
        )
    }
    ...
    moveArea({ x, y, width, height }) {
        ...
        let clampedWidth = Math.max(
            DocumentAlignmentWidget.DEFAULT_VALUES.MIN_SIZE,
            width,
        )
        let clampedHeight = Math.max(
            DocumentAlignmentWidget.DEFAULT_VALUES.MIN_SIZE,
            height,
        )
    ...
    setAreaPosition(value) {
        ...
        if (value) {
            ...
            this.imageWrapper.style.setProperty(
                CSS_PROPERTIES.AREA_TOP,
                `${top}%`,
            )
            this.imageWrapper.style.setProperty(
                CSS_PROPERTIES.AREA_RIGHT,
                `${right}%`,
            )
            this.imageWrapper.style.setProperty(
                CSS_PROPERTIES.AREA_BOTTOM,
                `${bottom}%`,
            )
            this.imageWrapper.style.setProperty(
                CSS_PROPERTIES.AREA_LEFT,
                `${left}%`,
            )
        } else {
    ...
    setHotspotPosition(value) {
        ...
        this.hotspot.style.top =
            typeof value?.y === 'number' ? `${value.y * 100}%` : ''
        this.hotspot.style.left =
            typeof value?.x === 'number' ? `${value.x * 100}%` : ''
    }
    ...
    getStoredHotspotData() {
        ...
        if (hotspot && hotspot !== 'null') {
            try {
                ...
                console.error(
                    'DocumentAlignmentWidget: Invalid stored hotspot data:',
                    error,
                )
            }
        }
    ...
    setHotspotData(value) {
        ...
        if (hotspotInput) {
            ...
            try {
                ...
                console.error(
                    'DocumentAlignmentWidget: Invalid existing hotspot data:',
                    error,
                )
            }
    ...
    startDragHotspot() {
        ...
        this.hotspot?.classList.add(
            DocumentAlignmentWidget.CSS_CLASSES.HOTSPOT_GRABBING,
        )
    ...
    onAreaPointerDown(event) {
        ...
        this.listen(document, 'pointermove', this.onPointerMove, {
            passive: false,
        })
    ...
    onPointerUp(event) {
        ...
        if (
            this.pointerDownEvent.clientX === event.clientX &&
            this.pointerDownEvent.clientY === event.clientY
        ) {
            this.moveHotspot(event.clientX, event.clientY)
        }
        ...
        this.unlisten(document, 'pointermove', this.onPointerMove)
        this.unlisten(document, 'pointerup', this.onPointerUp)
