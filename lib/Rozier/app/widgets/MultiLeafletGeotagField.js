...
export default class MultiLeafletGeotagField extends LeafletGeotagField {
    constructor() {
        ...
        this.$fields = document.querySelectorAll(
            '.rz-multi-geotag-field:not(.is-enable)',
        )
    ...
    bindSingleField(element) {
        ...
        const $label = element.parentElement.querySelector('.rz-form-field__head__label')
        ...
        let metaDOM = [
            ...
            '<div class="geotag-widget-nav__title label">' +
                labelText +
                '</div>',
            '</div>',
            '<div class="geotag-widget-nav__content">',
            '<div class="geotag-widget-quick-creation rz-button-group rz-button-group--gap-md">',
            '<input autocomplete="off" is="rz-input" class="rz-geotag-address" id="' +
                fieldAddressId +
                '" type="text" value="" />',
            '<button type="button" id="' +
                resetButtonId +
                '" class="rz-button rz-button--error rz-geotag-reset" title="' +
                window.RozierConfig.messages.geotag.resetMarker +
                '" data-uk-tooltip="{animation:true}"><span class="rz-button__icon rz-icon-ri--delete-bin-7-line"></span></button>',
            '</div>',
            '</div>',
            '</nav>',
        ...
        $geocodeInput.setAttribute(
            'placeholder',
            window.RozierConfig.messages.geotag.typeAnAddress,
        )
        // Reset button
        ...
        const markers = []
        /** @type {HTMLElement} */
        const $selector = element.parentElement.querySelector(
            '.multi-geotag-list-markers',
        )
        ...
        if (element.value !== '') {
            try {
                ...
                if (!featureCollection.features) {
                    throw new Error(
                        'Data is not a valid GeoJSON featureCollection',
                    )
                }
    ...
    syncSelector($selector, markers, map, $input) {
        ...
        const innerHtml = []
        let i = 0
        const actualMarkers = markers.filter(
            (marker) => marker !== null && marker.getLatLng() !== null,
        )
        ...
        let j = 0
        for (const marker of actualMarkers) {
            let $centerBtn = $selector.querySelector(
                '.rz-multi-geotag-center[data-geocode-id="' + j + '"]',
            )
            let $removeBtn = $selector.querySelector(
                '.rz-multi-geotag-remove[data-geocode-id="' + j + '"]',
            )
            if ($centerBtn && $removeBtn) {
                $centerBtn.addEventListener(
                    'click',
                    this.centerMap.bind(this, map, marker),
                )
                $removeBtn.addEventListener(
                    'click',
                    this.removeMarker.bind(
                        this,
                        map,
                        markers,
                        marker,
        ...
        for (const marker of markers) {
            if (marker) {
                ...
                featuresCollection.features.push(
                    this.latLngToFeature(latLng, latLng.alt, marker.name),
                )
            }
        }
