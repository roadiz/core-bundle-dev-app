...
export default class Rozier {
    ...
    initNestables() {
        ...
        document.querySelectorAll('.uk-nestable').forEach((element) => {
            ...
            if (element.classList.contains('node-tree')) {
                options.group = 'nodeTree'
            } else if (element.classList.contains('tag-tree')) {
                options.group = 'tagTree'
            } else if (element.classList.contains('folder-tree')) {
    ...
    bindMainTrees() {
        ...
        const tagTrees = document.querySelectorAll(
            '.tag-tree-widget .root-tree',
        )
    ...
    async refreshAsideTreeContent(treeHTML = '') {
        ...
        const temporalyElement = document.createElement('div')
        temporalyElement.innerHTML = treeHTML
        const newElement = temporalyElement.querySelector('.rz-tree-wrapper')
    ...
    async refreshMainTree(type = undefined, translationId = undefined) {
        ...
        let fetchUrl = window.RozierConfig.routes.treeAjaxGateway
        let responseHtmlKey = ''

        if (type === 'node') {
            fetchUrl = window.RozierConfig.routes.nodesTreeAjax
            responseHtmlKey = 'nodeTree'
        } else if (type === 'tag') {
            fetchUrl = window.RozierConfig.routes.tagsTreeAjax
            responseHtmlKey = 'tagTree'
        } else if (type === 'folder') {
            fetchUrl = window.RozierConfig.routes.foldersTreeAjax
            responseHtmlKey = 'folderTree'
        }
        ...
        const treeHTML =
            data?.[responseHtmlKey] ||
            data?.['nodeTree'] ||
            data?.['tagTree'] ||
            data?.['folderTree']

        if (typeof treeHTML !== 'undefined') {
            this.refreshAsideTreeContent(treeHTML)
        }
    }

    async refreshAsideMainTree(type = undefined, translationId = undefined) {
        try {
            await this.refreshMainTree(type, translationId)
        } catch {
    ...
    async refreshMainNodeTree(translationId = undefined) {
        this.refreshAsideMainTree('node', translationId)
    }

    async refreshMainTagTree(translationId = undefined) {
        this.refreshAsideMainTree('tag', translationId)
    }

    async refreshMainFolderTree(translationId = undefined) {
        this.refreshAsideMainTree('folder', translationId)
    }
