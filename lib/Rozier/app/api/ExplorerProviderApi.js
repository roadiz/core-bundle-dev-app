...
export function getItemsByIds({ ids = [], filters }) {
    ...
    return fetch(
        window.RozierConfig.routes.providerAjaxByArray +
            '?' +
 ...
 */
export function getItems({ searchTerms, preFilters, filters, moreData }) {
    const postData = {
        _token: window.RozierConfig.ajaxToken,
    ...
    return fetch(
        window.RozierConfig.routes.providerAjaxExplorer +
            '?' +
            new URLSearchParams(postData),
        {
            method: 'GET',
