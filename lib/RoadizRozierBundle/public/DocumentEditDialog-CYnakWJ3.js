import{R as u}from"./RoadizElement-XU473doh.js";class o extends u{static INPUT_BASE_NAME="document_edit_dialog[proxy]";static INPUT_KEYS=["imageCropAlignment","hotspot"];static CSS_SELECTORS={DIALOG_CLOSE:".document-edit-dialog__close",DIALOG_ACTIONS:".document-edit-dialog__actions > button",DIALOG_CONTENT:".document-edit-dialog__content",ALIGNMENT_WIDGET:"document-alignment-widget"};dialog=null;connectedCallback(){this.createDialog(),this.setupEventListeners(),this.appendToDOM(),this.loadTemplate(),this.hasAttribute("open")&&this.dialog.showModal()}createDialog(){this.dialog=document.createElement("dialog"),this.dialog.setAttribute("closedby","any"),this.dialog.classList.add("uk-form","document-edit-dialog"),this.dialog.innerHTML=this.getDialogHTML()}getDialogHTML(){const{INPUT_BASE_NAME:t,INPUT_KEYS:e}=o;return`
...
`)}
        ...
        `:""}showModal(){this.dialog?.showModal()}onButtonClick(t){(t.currentTarget.value==="cancel"||t.target.value==="submit")&&this.dialog.close(t.target.value)}onEditLinkClick(t){t.preventDefault(),this.dialog.close("cancel"),window.Rozier.lazyload.onClick(t)}onDialogClose(){const{INPUT_BASE_NAME:t}=o;this.dialog.returnValue==="submit"&&this.applyInputValues(t,this.getAttribute("input-base-name"),this.dialog,document),this.remove()}}export{o as default};
