...
export default class SettingsSaveButtons {
    constructor() {
        // Selectors
        this.button = document.querySelector('.uk-button-settings-save')
        ...
        if (!response.ok) {
            ...
            if (data.errors && data.errors.value) {
                for (let key in data.errors.value) {
                    ...
                    form.insertAdjacentHTML('beforeend', '<span class="uk-alert uk-alert-danger">' + data.errors.value[key] + '</span>')
                }
            } else if (data.errors) {
                for (let key in data.errors) {
                    form.classList.add('uk-has-errors')
                    form.insertAdjacentHTML('beforeend', '<span class="uk-alert uk-alert-danger">' + data.errors[key] + '</span>')
                    break
                }
            }
        }
