...
export default class DocumentAlignmentWidget extends HTMLElement {
    ...
    updateElementsPositionsFromStoredData() {
        let data = this.getStoredHotspotData()

        this.toggleResetButtonVisibility(!!data)

        console.log('DocumentAlignmentWidget: Loaded hotspot data:', data)

        if (!data) {
            // If no stored data, use initial hotspot if available
            const originalHotspot = this.getAttribute('original-hotspot')

            console.log('DocumentAlignmentWidget: Using initial hotspot:', originalHotspot)

            if (originalHotspot) {
                try {
                    data = JSON.parse(originalHotspot)
                } catch (e) {
                    console.error('Invalid initial hotspot data:', e)
                    data = null
                }
            }
        }
