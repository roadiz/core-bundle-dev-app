export default class AdminMenuNav extends HTMLElement {
    connectedCallback() {
        ...
        this.querySelectorAll('ul[role="menu"]').forEach((element) => {
            ...
            menuItem.addEventListener('mouseenter', () => this.enterMenuItem(menuItem))
            menuItem.addEventListener('focusin', () => this.enterMenuItem(menuItem))
            menuItem.addEventListener('mouseleave', () => this.leaveMenuItem(menuItem))
            menuItem.addEventListener('focusout', () => this.leaveMenuItem(menuItem))
        })
