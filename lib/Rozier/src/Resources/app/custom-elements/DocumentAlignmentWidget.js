...
export default class DocumentAlignmentWidget extends HTMLElement {
    ...
    connectedCallback() {
        ...
        if (originalHotspot) {
            try {
                const data = JSON.parse(originalHotspot)

                this.originalHotspot = data
            } catch (error) {
                console.error('Invalid original hotspot data:', error)
            }
        }

        this.overridableHotspot = Boolean(this.getAttribute('overridable-hotspot'))

        this.init()
        this.updateImagePath()

        if (this.overridableHotspot) {
            this.overrideButton.classList.add('document-alignment-widget__override--visible')
            this.overrideButtonInput.checked = !!storedData

            this.area.inert = !this.overrideButtonInput.checked

            this.overrideButtonInput.addEventListener('change', (event) => {
    ...
    init() {
        ...
        this.handles = Array.from(this.querySelectorAll('.document-alignment-widget__handle'))
        ...
        this.overrideButton = this.querySelector('.document-alignment-widget__override')
        ...
        this.resetButton?.addEventListener('click', () => {
            const newHotspotData = this.overridableHotspot
                ? { x: 0.5, y: 0.5, areaStartX: 0, areaStartY: 0, areaEndX: 1, areaEndY: 1 }
                : null
    ...
    moveArea({ x, y, width, height }) {
        ...
        const data = {
            areaStartX: round(normLeft),
            areaStartY: round(normTop),
            areaEndX: round(normLeft + normWidth),
            areaEndY: round(normTop + normHeight),
        }
    ...
    setAreaPosition(value) {
        if (value) {
            ...
            this.imageWrapper.style.setProperty('--document-alignment-widget-area-top', `${top}%`)
            this.imageWrapper.style.setProperty('--document-alignment-widget-area-right', `${right}%`)
            this.imageWrapper.style.setProperty('--document-alignment-widget-area-bottom', `${bottom}%`)
            this.imageWrapper.style.setProperty('--document-alignment-widget-area-left', `${left}%`)
        } else {
            this.imageWrapper.style.removeProperty('--document-alignment-widget-area-top')
            this.imageWrapper.style.removeProperty('--document-alignment-widget-area-right')
            this.imageWrapper.style.removeProperty('--document-alignment-widget-area-bottom')
            this.imageWrapper.style.removeProperty('--document-alignment-widget-area-left')
        }
    }
