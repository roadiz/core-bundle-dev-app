...
export default class LeafletGeotagField {
    ...
    bindSingleField(element) {
        ...
        let metaDOM = [
            ...
            '<input autocomplete="off" is="rz-input" id="' +
                fieldAddressId +
                '" type="text" value="" />',
        ...
        map.on(
            'click',
            this.setMarkerEvent.bind(this, marker, element, $geocodeReset, map),
        )
        $geocodeInput.addEventListener(
            'keypress',
            this.requestGeocode.bind(this, marker, element, $geocodeReset, map),
        )
    ...
    setMarkerEvent(marker, $input, $geocodeReset, map, event) {
        if (!marker) {
            marker = this.createAndBindMarker(
                event.latlng,
                map,
                $input,
                $geocodeReset,
    ...
    createAndBindMarker(latlng, map, $input, $geocodeReset) {
        ...
        marker.on(
            'dragend',
            this.setMarkerEvent.bind(this, marker, $input, $geocodeReset, map),
        )
        $geocodeReset.addEventListener(
            'click',
            this.resetMarker.bind(this, marker, $input, $geocodeReset, map),
        )
    ...
    createMap(fieldId, mapOptions) {
        const map = new Map(document.getElementById(fieldId)).setView(
            mapOptions.center,
            mapOptions.zoom,
        )
    ...
    createLatLng(data) {
        ...
        if (typeof data.lat === 'number' && typeof data.lng === 'number') {
            ...
            const zoom =
                data.properties && data.properties.zoom
                    ? Number.parseInt(data.properties.zoom)
                    : 7
