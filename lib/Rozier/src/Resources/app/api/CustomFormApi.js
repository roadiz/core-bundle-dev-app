...
export function getCustomFormsByIds({ ids = [], filters }) {
    ...
    return fetch(window.RozierRoot.routes.customFormsAjaxByArray + '?' + new URLSearchParams(postData), {
        method: 'GET',
        headers: {
...
export function getCustomForms({ searchTerms, filters, moreData }) {
    ...
    return fetch(window.RozierRoot.routes.customFormsAjaxExplorer + '?' + new URLSearchParams(postData), {
        method: 'GET',
        headers: {
            accept: 'application/json',
        },
