...
export function getJoinsByIds({ ids = [], filters }) {
    ...
    return fetch(window.RozierRoot.routes.joinsAjaxByArray + '?' + new URLSearchParams(postData), {
        method: 'GET',
        headers: {
...
export function getJoins({ searchTerms, preFilters, filters, filterExplorerSelection, moreData }) {
    ...
    return fetch(window.RozierRoot.routes.joinsAjaxExplorer + '?' + new URLSearchParams(postData), {
        method: 'GET',
        headers: {
            accept: 'application/json',
        },
