...
export default class NodeEditSource {
    ...
    wrapInTabs() {
        ...
        if (fieldsLength > 0) {
            ...
            if (fieldsGroupsLength > 1) {
                ...
                let formSwitcher = this.form.querySelector('.uk-switcher')
                let formSwitcherNav = this.form.querySelector('#node-source-form-switcher-nav')
                ...
                for (let index in fieldGroups) {
                    ...
                    if (fieldGroup.id === 'default') {
                        formSwitcherNav.insertAdjacentHTML('beforeend',
                ...
                formSwitcherNav.addEventListener('shown.uk.switcher', () => {
                    window.requestAnimationFrame(() => {
                        window.dispatchEvent(new CustomEvent('NodeEditSource:tabSwitched'))
                        this.$window.addEventListener('resize', this.resize)
                        window.Rozier.$window.dispatchEvent(new Event('resize'))
    ...
    init() {
        ...
        this.input = this.content.querySelector('input, select')
        ...
        for (let j = this.devNames.length - 1; j >= 0; j--) {
            ...
            if (input.getAttribute('data-dev-name') !== '') {
                let label = input.closest('.uk-form-row').querySelector('label')
                let barLabel = input.querySelector('.uk-navbar-brand.label')

                if (label) {
                    label.insertAdjacentHTML('beforeend', '<span class="field-dev-name">' + input.getAttribute('data-dev-name') + '</span>')
                } else if (barLabel) {
                    barLabel.insertAdjacentHTML('beforeend', '<span class="field-dev-name">' + input.getAttribute('data-dev-name') + '</span>')
                }
            }
        }
    ...
    initEvents() {
        window.Rozier.$window.addEventListener('keydown', this.onInputKeyDown)
        window.Rozier.$window.addEventListener('keyup', this.onInputKeyUp)
        this.input.addEventListener('focus', this.inputFocus)
        this.input.addEventListener('focusout', this.inputFocusOut)
        this.form.addEventListener('submit', this.onFormSubmit)
    }

    unbind() {
        if (this.content) {
            window.Rozier.$window.removeEventListener('keydown', this.onInputKeyDown)
            window.Rozier.$window.removeEventListener('keyup', this.onInputKeyUp)
            this.input.removeEventListener('focus', this.inputFocus)
            this.input.removeEventListener('focusout', this.inputFocusOut)
    ...
    async onFormSubmit(event) {
        ...
        try {
            ...
            if (data.public_url) {
                let publicUrlLinks = document.querySelectorAll('a.public-url-link')
                if (publicUrlLinks) {
                    publicUrlLinks.getAttribute('href', data.public_url)
                    publicUrlLinks.forEach(link => link.setAttribute('href', data.public_url))
                }
            }
            if (data.preview_url) {
                let previewUrlLinks = document.querySelectorAll('a.preview-url-link')
    ...
    onInputKeyDown(event) {
        // ALT key
        if (event.keyCode === 18) {
            window.Rozier.$body.classList.toggle('dev-name-visible')
        }
    }
