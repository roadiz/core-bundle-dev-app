...
export default class ChildrenNodesField {
    constructor() {
        ...
        this.nodeTrees = document.querySelectorAll(
            '[data-children-nodes-widget] .nodetree-widget',
        )
        this.quickAddNodeButtons = document.querySelectorAll(
            '.children-nodes-quick-creation button',
        )
    ...
    cleanNodeTree() {
        ...
        document
            .querySelectorAll('[data-children-nodes-widget] .nodetree-langs')
            .forEach((element) => {
                element.remove()
            })
        this.quickAddNodeButtons.forEach((button) => {
    ...
    async quickAddNode(nodeTypeName, parentNodeId, translationId) {
        const response = await fetch(
            window.RozierConfig.routes.nodesQuickAddAjax,
            {
                method: 'POST',
                headers: {
    ...
    async onQuickAddClick(event) {
        ...
        if (nodeTypeName !== '' && parentNodeId > 0) {
            try {
                ...
                let nodeTree = link
                    .closest('.children-nodes-widget')
                    .querySelector('.nodetree-widget')
    ...
    async refreshNodeTree(nodeTree) {
        ...
        const rootNodeId = parseInt(
            rootTree.getAttribute('data-parent-node-id'),
        )
